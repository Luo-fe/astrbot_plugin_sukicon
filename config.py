...
class PluginConfig(ConfigNode):
    ...
    def save_dedup(self):
        ...
        if not self.enable_deduplication:
            return
        pids_list = list(self._sent_pids)[-self.dedup_history_size:]
        try:
    ...
    def mark_pid_sent(self, pid: int):
        if not self.enable_deduplication:
            return
        self._sent_pids.add(pid)
        if len(self._sent_pids) > self.dedup_history_size * 2:
            self._sent_pids = set(list(self._sent_pids)[-self.dedup_history_size:])
        self.save_dedup()
